...
export default function VenueSection() {
  ...
  return (
    <section id="venue" className="bg-[#F8F5F0] py-20 px-6">
      <div className="mx-auto max-w-5xl">
        ...
        >
          <h2 className="font-heading text-3xl text-gray-800">Venue & Map</h2>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          <motion.div
            className="rounded-2xl border border-[#D4AF37]/20 bg-white/70 p-6 shadow-xl backdrop-blur-sm"
            variants={fadeUp}
            custom={1}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.2 }}
          >
            <h3 className="font-heading text-2xl text-gray-800">{invitationData.venue.name}</h3>
            <p className="font-body mt-1 text-sm text-[#6B6B6B]">{invitationData.venue.subtitle}</p>

            <div className="mt-6 space-y-4">
              <div className="flex items-start gap-3">
                <MapPin className="mt-0.5 h-5 w-5 text-[#D4AF37]" />
                <p className="font-body text-sm text-gray-700">{invitationData.venue.address}</p>
              </div>
              <div className="flex items-start gap-3">
                <CalendarDays className="mt-0.5 h-5 w-5 text-[#D4AF37]" />
                <p className="font-body text-sm text-gray-700">{invitationData.eventDateLabel}</p>
              </div>
              <div className="flex items-start gap-3">
                <Clock3 className="mt-0.5 h-5 w-5 text-[#D4AF37]" />
                <p className="font-body text-sm text-gray-700">
                  Prayer starts at <span className="font-semibold text-[#B8962E]">{invitationData.venue.prayerStart}</span>
                </p>
              </div>
            </div>
